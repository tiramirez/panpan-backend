...
def test_weekly_orders_returns_pivot(s3_bucket, dynamodb_table):
    ...
    seed_order(dynamodb_table, "2026-25", "order-001", "alice@example.com", [
        {"product_name": "Apples", "product_quantity": 2, "unit_price": Decimal("3.00")},
        {"product_name": "Oranges", "product_quantity": 1, "unit_price": Decimal("2.00")},
    ])
    seed_order(dynamodb_table, "2026-25", "order-002", "bob@example.com", [
        {"product_name": "Apples", "product_quantity": 3, "unit_price": Decimal("3.00")},
    ])
    ...
    assert apples_col in data, f"Expected '{apples_col}' in data keys: {list(data.keys())}"
...
def test_weekly_orders_different_weeks_isolated(s3_bucket, dynamodb_table):
    ...
    seed_order(dynamodb_table, "2026-25", "order-w25", "w25@example.com", [
        {"product_name": "Apples", "product_quantity": 1, "unit_price": Decimal("3.00")},
    ])
    seed_order(dynamodb_table, "2026-26", "order-w26", "w26@example.com", [
        {"product_name": "Apples", "product_quantity": 2, "unit_price": Decimal("3.00")},
    ])
